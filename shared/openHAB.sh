...
#    log_tool -t 1 -a "Reniced karaf process "${QPKG_PID}" to "$(awk '{print $19}' /proc/${QPKG_PID}/stat)
  ...
  mstart)
 # 	Immediately start and do not include wait/sleep
    setupEnvironment
    checkPorts
...
#    log_tool -t 1 -a "Reniced karaf process "${QPKG_PID}" to "$(awk '{print $19}' /proc/${QPKG_PID}/stat)
    ...
    # TODO: WORKAROUND: Waiting one and a half minute until the service is properly turned on
    echo "Started manually, check if openhab is really active"
    ;;
...
#    fi
  ...
  mstop)
# 	Immediately stop and do not wait/sleep for   
    setupEnvironment
#    if [ -f ${QPKG_PIDFILE} ]; then
#        kill -9 $(cat ${QPKG_PIDFILE}) > ${QPKG_STDOUT}_kill 2> ${QPKG_STDERR}_kill
#        rm ${QPKG_PIDFILE}
...
#    fi
    ...
    # TODO: WORKAROUND: Waiting one minute until the service is properly turned off
    echo  "stopped, check if openhab is really stopped."
    ;;
  ...
  ports)
    # added by pocs on 13jun18 for test ,show used http(s) ports
    setupEnvironment
  ...
  downloadJavaX64)
    ...
    ;;
  *)
    echo "Usage: $0 {start|mstart|stop|mstop|restart|ports|status|console|backup|snapshot-update|snapshot-download|downloadJava}"
    exit 1
esac
